...
def delete_word_rule(target, rules):
    """
...
def word_to_number(s):
    """
    方法 translate_number 的辅助方法，可将[零-九]正确翻译为[0-9]
    :param s: 大写数字
...
def translate_number(target):
    ...
    for m in match:
        ...
        num = 0
        if len(s) == 2:
            num += word_to_number(s[0]) * 10000 + word_to_number(
                s[1]) * 1000
    ...
    for m in match:
        ...
        num = 0
        if len(s) == 2:
            num += word_to_number(s[0]) * 1000 + word_to_number(
                s[1]) * 100
        target = pattern.sub(str(num), target, 1)
    ...
    for m in match:
        ...
        num = 0
        if len(s) == 2:
            num += word_to_number(s[0]) * 100 + word_to_number(
                s[1]) * 10
        target = pattern.sub(str(num), target, 1)

    pattern = regex.compile(u"[零一二两三四五六七八九]")
    match = pattern.finditer(target)
    for m in match:
        target = pattern.sub(str(word_to_number(m.group())), target, 1)

    pattern = regex.compile(u"(?<=(周|星期))[末天日]")
    match = pattern.finditer(target)
    for m in match:
        target = pattern.sub(str(word_to_number(m.group())), target, 1)

    pattern = regex.compile(u"(?<!(周|星期))0?[0-9]?十[0-9]?")
    match = pattern.finditer(target)
    for m in match:
        group = m.group()
        s = group.split(u"十")
    ...
    pattern = regex.compile(u"0?[1-9]百[0-9]?[0-9]?")
    match = pattern.finditer(target)
    for m in match:
        group = m.group()
        s = group.split(u"百")
    ...
    pattern = regex.compile(u"0?[1-9]千[0-9]?[0-9]?[0-9]?")
    match = pattern.finditer(target)
    for m in match:
        group = m.group()
        s = group.split(u"千")
    ...
    pattern = regex.compile(u"[0-9]+万[0-9]?[0-9]?[0-9]?[0-9]?")
    match = pattern.finditer(target)
    for m in match:
        group = m.group()
        s = group.split(u"万")
